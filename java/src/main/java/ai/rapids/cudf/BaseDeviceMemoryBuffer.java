...
public abstract class BaseDeviceMemoryBuffer extends MemoryBuffer {
  ...
  private static boolean BOOKEEP_MEMORY = Boolean.getBoolean("ai.rapids.memory.bookkeep");
  private static final ConcurrentHashMap<Long, LongAdder> deviceMemPerThread = new ConcurrentHashMap<>();
  private static final ConcurrentHashMap<Long, String> threadId2ThreadName = new ConcurrentHashMap<>();
  ...
  public static String getDeviceMemoryBookkeepSummary() {
    if(BOOKEEP_MEMORY) {
      ...
      for (Map.Entry<Long, LongAdder> entry : deviceMemPerThread.entrySet()) {
        ...
        String threadName = threadId2ThreadName.get(threadId);
        sb.append("Thread ").append(threadId).append(" (").append(threadName).append(") has ")
  ...
  public static class OnCloseCallback2 implements EventHandler {
    private long ptr;
    private long amount;

    public OnCloseCallback2(long ptr, long amount) {
      this.ptr = ptr;
      this.amount = amount;
    }
        ...
        if (BOOKEEP_MEMORY) {
          ...
          if (threadId != null) {
            ...
            if (adder != null) {
              ...
              log.warn("Could not find adder for thread " + threadId + " from address " + ptr +
                  " bytes: " + amount);
            }
            addr2threadId.remove(ptr);
          } else {
            log.warn("Could not find thread id for address " + ptr + " bytes: " + amount);
          }
        }
      }
    }
  ...
  protected BaseDeviceMemoryBuffer(long address, long length, MemoryBufferCleaner cleaner) {
    ...
    if (BOOKEEP_MEMORY && cleaner != null && !(cleaner instanceof SlicedBufferCleaner)) {
      long threadId = Thread.currentThread().getId();
      addr2threadId.put(address, threadId);
      bookkeepDeviceMemory(threadId, length);
      setEventHandler2(new OnCloseCallback2(address, length));
    }
  }
